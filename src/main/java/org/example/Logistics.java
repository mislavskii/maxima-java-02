...
public class Logistics {
    ...
    public Transport getShipping (City city, int weight, int time) {
        ...
        for (int i=0; i < vehicles.length; i++) {
            float totalPrice = getTotalPrice(vehicles[i], city, weight, time);
            if (totalPrice > 0) {
        ...
        if (startIndex < vehicles.length - 1) {
            for (Transport vehicle : java.util.Arrays.copyOfRange(vehicles, startIndex, vehicles.length - 1)) {
                float totalPrice = getTotalPrice(vehicle, city, weight, time);
    ...
    float getTotalPrice (Transport vehicle, City city, int weight, int time) {
